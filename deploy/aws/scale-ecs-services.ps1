...
Write-Host "Requested ECS desired counts:"
foreach ($entry in $counts.GetEnumerator()) {
  Write-Host ("  {0,-12} {1}" -f $entry.Key, $entry.Value)
}

if (-not $Apply) {
  Write-Host "`nDry run only. Add -Apply to update ECS services."
  return
}

$updatedServices = @()
foreach ($entry in $counts.GetEnumerator()) {
  $serviceName = "$ProjectName-$EnvironmentName-$($entry.Key)"
...
if ($Wait) {
  Write-Host "`nWaiting for ECS services to stabilize..."
  & aws @awsArgs ecs wait services-stable `
    --cluster $ClusterName `
    --services $updatedServices

  if ($LASTEXITCODE -ne 0) {
    throw "ECS services did not stabilize."
  }
}
