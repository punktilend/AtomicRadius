param(
  [switch]$Apply,
  [string]$StackName = "atomic-radius-edge-alb",
...
$template = Join-Path $scriptDir "edge-alb.yaml"
...
if (-not $Apply) {
  Write-Host "Dry run only. Re-run with -Apply to create billable ALB resources."
  return
}
...
$albSecurityGroupId = Get-FoundationOutput "AlbSecurityGroupId"
